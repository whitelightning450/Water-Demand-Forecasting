...
class CSD_WDM(object):
    ...
    def Demand_Optimization(self, Param_dict, scoring, figpath):   
        ...
        pbar = ProgressBar()
        for month in pbar(Outdoor_Months):
        ...
        #calibrate indoor months with the indoor demand model
        ValDF = self.Indoor_Demand_Model(ValDF)
        ForecastDF = self.Indoor_Demand_Model(ForecastDF) ################
    ...
    def Indoor_Demand_Model(self, df): 
        ...
        for month in IndoorMonths:
            ...
            colrem = df[month].columns
            
            df[month]=df[month].reset_index(drop=True)
            
            df[month]['Year'] = df[month]['year']
            df[month]['y_pred'] =  df[month]['cons_goal'] ############### just sets prediction as goal-- what about when no goal
            df[month]['y_test'] =  df[month]['Target_gpcd']
            df[month]['y_pred_tot'] =  df[month]['cons_goal']
            df[month]['y_test_tot'] =  df[month]['Target_gpcd']

            df[month].index = df[month]['Date']
